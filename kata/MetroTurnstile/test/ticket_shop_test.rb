...
class TicketShopTest < Test::Unit::TestCase
  ...
  def test_get_one_way_ticket
    ...
    assert_true ticket.scan_at(Date.today).include?('BEEP OK')
  end

  def test_get_daily_ticket
    ticket = TicketShop.new.get('DAILY')

    assert_true ticket.scan_at(Date.today).include?('BEEP DAILY OK')
  end

  def test_get_weekly_ticket
    ticket = TicketShop.new.get('WEEKLY')

    assert_true ticket.scan_at(Date.today).include?('BEEP WEEKLY OK')
  end
